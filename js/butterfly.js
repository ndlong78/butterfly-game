import { CANVAS, COLORS, GAME, LEVELS } from './config.js';
...
export class Butterfly {
  ...
  update(dt) {
    if (this.caught) {
      if (this.effectMs > 0) {
        this.effectMs = Math.max(0, this.effectMs - dt);
      }
      return;
    }

    this.elapsed += dt;
    if (this.elapsed >= this.segmentMs) {
      this._nextSegment();
      this.elapsed = 0;
    }
  ...
  _drawCaughtEffect(ctx) {
    if (this.effectMs <= 0) {
      return;
    }

    const progress = 1 - this.effectMs / WING_EFFECT_MS;
    ...
    for (let i = 0; i < 8; i += 1) {
      ...
      ctx.translate(sx, sy);
      ctx.rotate(angle);
      ctx.fillStyle = i % 2 === 0 ? '#FFD54A' : '#FFB347';
      ctx.beginPath();
  ...
  draw(ctx) {
    ...
    wingGradient.addColorStop(0.45, this.color);
    wingGradient.addColorStop(1, '#A45CB8');
    ...
    ctx.beginPath();
    ctx.moveTo(0, 0);
    ctx.bezierCurveTo(size * 0.2, -size * 0.95, size * 1.15, -size * 0.45, size * 0.25, 0);
    ctx.bezierCurveTo(size * 0.95, size * 0.4, size * 0.15, size * 0.8, 0, 0);
    ctx.closePath();
    ctx.fill();
    ...
    ctx.beginPath();
    ctx.moveTo(0, 0);
    ctx.bezierCurveTo(lowerSize * 0.2, lowerSize * 0.2, lowerSize * 0.8, lowerSize * 0.95, lowerSize * 0.18, lowerSize * 1.05);
    ctx.bezierCurveTo(lowerSize * 0.5, lowerSize * 0.1, lowerSize * 0.1, -lowerSize * 0.2, 0, 0);
    ctx.closePath();
    ctx.fill();
    ...
    ctx.fillStyle = '#3F2A50';
    ctx.beginPath();
    ctx.ellipse(0, -size * 0.2, size * 0.1, size * 0.14, 0, 0, Math.PI * 2);
    ctx.ellipse(0, 0, size * 0.11, size * 0.18, 0, 0, Math.PI * 2);
    ctx.ellipse(0, size * 0.25, size * 0.1, size * 0.15, 0, 0, Math.PI * 2);
    ctx.fill();

    ctx.strokeStyle = '#3F2A50';
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(-size * 0.05, -size * 0.32);
    ctx.quadraticCurveTo(-size * 0.25, -size * 0.7, -size * 0.1, -size * 0.88);
    ctx.moveTo(size * 0.05, -size * 0.32);
    ctx.quadraticCurveTo(size * 0.25, -size * 0.7, size * 0.1, -size * 0.88);
    ctx.stroke();
    ...
    ctx.fillStyle = '#3F2A50';
    ctx.beginPath();
    ctx.arc(-size * 0.1, -size * 0.88, 2.5, 0, Math.PI * 2);
    ctx.arc(size * 0.1, -size * 0.88, 2.5, 0, Math.PI * 2);
    ctx.fill();
